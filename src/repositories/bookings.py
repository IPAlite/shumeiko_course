...
class BookingsRepository(BaseRepository):
    ...
    async def has_overlap(
        self, hotel_id: int, room_id: int, date_from: date, date_to: date
    ) -> bool:
        query = select(
            exists().where(
                and_(
    ...
    async def add_booking(self, data: BookindAdd):
        rooms_ids_for_get = rooms_ids_for_booking(
            hotel_id=data.hotel_id, date_from=data.date_from, date_to=data.date_to
        )
        query = select(RoomsOrm).filter(
            RoomsOrm.id.in_(rooms_ids_for_get), RoomsOrm.hotel_id == data.hotel_id
        )
